...
export class EntriesFacadeService {
    ...
    this.store
      ...
      .subscribe(ew => {
        if (
          ...
          this.store.dispatch(
            new LoadEntriesAction({ startDate: new Date(this.timestamp), endDate: new Date(endDate) })
          );
        }
      });
  }
  ...
  public OnAddEntryClick(): void {
    this.store.dispatch(new ShowAddEntryAction({ date: new Date(this.timestamp) }));
  }
}
